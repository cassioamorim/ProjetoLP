...
public class DAOProduto extends DAOGenerico<Produto> {
    ...
    public List<Produto> listByNome(String nome) {
        return em.createQuery("SELECT e FROM Produto e WHERE e.nome LIKE :nome").setParameter("nome", "%" + nome + "%").getResultList();
    }
    ...
    public List<String> listInOrderNomeStrings(String qualOrdem) {
        ...
        for (int i = 0; i < lf.size(); i++) {
            ls.add(lf.get(i).getIdProduto() + "-" + lf.get(i).getNome());
        }
        return ls;
    }
